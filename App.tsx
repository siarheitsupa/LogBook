...
import { formatMinsToHHMM, getStats, calculateLogSummary, getMonday } from './utils/timeUtils';
import { validateDrivingCycle } from './utils/complianceUtils'; // Импорт валидации
...
const App: React.FC = () => {
  ...
  const enrichedData = useMemo(() => {
    ...
    // Запуск новой валидации комплаенса
    const violations = validateDrivingCycle(shifts);

    const shiftsWithData = summary.shifts.map(s => ({
      ...s,
      expenses: expenses.filter(e => e.shiftId === s.id),
      violation: violations[s.id] // Добавляем текст нарушения в объект смены
    }));
    return { ...summary, shifts: shiftsWithData };
  }, [shifts, expenses]);
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pb-24 font-sans text-slate-900 px-4 pt-6">
      {/* Header Capsule - Professional Edition Original */}
      ...
      <main className="max-w-md mx-auto space-y-8">
        ...
        <button 
          ...
          className={`w-full ${appState.isActive ? 'bg-rose-500' : 'bg-[#10b981]'} text-white rounded-full py-8 px-10 flex justify-between items-center shadow-2xl shadow-emerald-200/50 active:scale-[0.98] transition-all group`}
        >
          <span className="text-2xl font-black uppercase tracking-tight ml-4">
      ...
      <ShiftModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSave={async (s) => { await storage.saveShift(s); loadData(); setIsModalOpen(false); if (!editingShift) { setAppState({isActive:false, startTime:null}); storage.clearState(); } }} initialData={editingShift} />
