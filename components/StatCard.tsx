...
const StatCard: React.FC<StatCardProps> = ({ label, value, sublabel, variant }) => {
  ...
  const pillStyles = {
    ...
    blue: 'bg-white/80 text-[#3b82f6]',
    purple: 'bg-white/80 text-[#9333ea]',
    orange: 'bg-white/80 text-[#ea580c]',
    rose: 'bg-white/80 text-[#e11d48]',
    emerald: 'bg-white/80 text-[#10b981]',
    indigo: 'bg-white/80 text-[#6366f1]',
  };

  return (
    <div className={`p-8 rounded-[2.5rem] border shadow-sm flex flex-col items-center justify-center text-center transition-all active:scale-95 ${styles[variant]}`}>
      <span className="text-3xl font-black mb-1 tracking-tighter tabular-nums text-slate-800">
        {value}
      </span>
      ...
      {sublabel && (
        <span className={`text-[8px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full border border-black/5 shadow-sm ${pillStyles[variant]}`}>
          {sublabel}
        </span>
      )}
    </div>
