      ...
      <div className="bg-white w-full max-w-md rounded-[2.8rem] shadow-2xl animate-in zoom-in duration-200 overflow-hidden border border-slate-100">
        <div className="p-7">
          <div className="flex justify-between items-center mb-8">
            <h3 className="text-xl font-black text-slate-800 uppercase tracking-tighter">Добавить расход</h3>
          ...
          <div className="flex gap-2 p-1.5 bg-slate-50 rounded-2xl mb-6 overflow-x-auto no-scrollbar">
            {CURRENCIES.map(c => (
              <button key={c} onClick={() => setCurrency(c)} className={`px-5 py-2.5 rounded-xl text-[10px] font-black transition-all ${currency === c ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-100'}`}>
                {c}
              </button>
            ))}
          </div>
          ...
          <div className="relative mb-8">
            <input 
              type="number" 
              placeholder="0.00" 
              className="w-full text-5xl font-black text-center py-8 bg-slate-50 rounded-[2rem] outline-none focus:bg-white focus:ring-4 ring-slate-50 transition-all tabular-nums" 
              value={amount} 
              onChange={e => setAmount(e.target.value)} 
              inputMode="decimal"
            />
            <span className="absolute bottom-4 right-8 text-[10px] font-black text-slate-300 uppercase">{currency}</span>
          </div>
          ...
          <div className="flex gap-3">
            <button onClick={onClose} className="flex-1 py-4 text-[10px] font-black uppercase text-slate-400 bg-slate-50 rounded-2xl">Отмена</button>
            <button 
              onClick={() => {
                const val = parseFloat(amount);
                if (val > 0) onSave({ id: Date.now().toString(), shiftId, category, amount: val, currency, timestamp: Date.now(), description });
              }} 
              className="flex-2 py-4 px-10 text-[10px] font-black uppercase text-white bg-slate-900 rounded-2xl shadow-xl shadow-slate-200"
            >
              Сохранить
            </button>
          </div>
