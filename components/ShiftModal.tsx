    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm safe-p-bottom">
      <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl animate-in fade-in zoom-in duration-200 flex flex-col max-h-[92dvh] overflow-hidden">
        <div className="p-4 sm:p-6 overflow-y-auto">
          <h3 className="text-xl font-black text-center mb-4 text-slate-800">Запись смены</h3>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 ml-1">Дата</label>
              <input 
                type="date" 
                className="w-full p-3 bg-slate-50 border border-slate-100 rounded-2xl text-center font-bold text-slate-700 focus:bg-white focus:ring-2 ring-blue-500/20 outline-none transition-all appearance-none" 
                value={date} 
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="min-w-0">
                ...
                <input 
                  type="time" 
                  className="w-full p-3 px-1.5 bg-slate-50 border border-slate-100 rounded-2xl text-center font-bold text-slate-700 focus:bg-white focus:ring-2 ring-blue-500/20 outline-none min-w-0" 
                  value={start} 
            ...
            <div className="flex gap-2 pt-2">
              <button 
                type="button" 
                onClick={onClose}
                className="flex-1 py-3.5 font-black text-slate-400 bg-slate-50 rounded-2xl active:bg-slate-100 transition-all text-[11px] uppercase tracking-widest"
              >
                Отмена
              </button>
              <button 
                type="submit" 
                className="flex-1 py-3.5 font-black text-white bg-blue-600 rounded-2xl active:scale-[0.98] shadow-xl shadow-blue-100 transition-all text-[11px] uppercase tracking-widest"
              >
                Сохранить
              </button>
            </div>
